...
dummy_score = []
for k in range(10):
    dummy_score.append(clf.score(X_test, np.reshape(Y_test.values, Y_test.shape[0])))

res = np.array(dummy_score)
print("mean : {}, max : {}, min : {}, std : {}".format(res.mean(), res.max(), res.min(), res.std()))
